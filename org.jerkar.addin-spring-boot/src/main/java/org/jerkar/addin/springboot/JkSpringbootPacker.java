...
public class JkSpringbootPacker {
	...
	private JkSpringbootPacker(JkDependencyResolver dependencyResolver,
			JkVersionedModule versionedModule, String loaderVersion,
			JkManifest manifestToMerge) {
		super();
	...
	public static final JkSpringbootPacker of(JkDependencyResolver resolver,
	...
	public void makeBootJar(File original, File target) {
		...
		String archiveName = JkUtilsString.substringBeforeLast(
				target.getName(), ".");
		JkFileTree springBootArchiveDir = JkFileTree.of(target.getParentFile())
				.from(archiveName);
		...
		// Find main class
		String className = JkClassLoader.findMainClass(springBootArchiveDir
				.root());

		// Add loader
		JkDependencies bootloaderDep = JkDependencies.builder()
				.on(JkSpringModules.Boot.LOADER, loaderVersion).build();
		JkPath loaderLibs = dependencyResolver.withDependencies(bootloaderDep)
				.get();
		JkZipper.unzip(loaderLibs, springBootArchiveDir.root());

		// Create manifest
		springBootArchiveDir.include("META-INF/**").deleteAll();
		manifest(original, className).writeTo(
				springBootArchiveDir.file("META-INF/MANIFEST.MF"));
	...
	private void _makeBootJar2(File original, File target) throws IOException {
		...
		for (File nestedJar : libs) {
			jarWriter.writeNestedLibrary("libs", nestedJar);
		}
	...
	private JkManifest manifest(File original, String startClassName) {
		JkManifest result = JkUtilsObject.firstNonNull(
				JkManifest.ofArchive(original), JkManifest.empty());
	...
	private File executableJar(File original) {
		String name = JkUtilsString
				.substringBeforeLast(original.getName(), ".");
